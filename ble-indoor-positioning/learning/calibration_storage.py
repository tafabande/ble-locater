...
===========================
Handles persistent serialization and deserialization of learned path-loss parameters (eta, bias, sample counts)
and metadata (building_name, firmware_version, training_sessions) to JSON.
"""
...
class CalibrationStorage:
    ...
    def load(cls, filepath: str = None) -> dict:
        ...
        if os.path.exists(target_path):
            try:
                ...
                logger.error(f"Failed to load calibration storage file: {e}")
        return {
            "version": 1,
            "building_name": "Hospital Main Wing (Floor 1)",
            "firmware_version": "2.4.0-esp32",
            "training_sessions": 1,
            "anchors": {},
            "total_samples": 0,
    ...
    def save(cls, learner, filepath: str = None, building_name: str = "Hospital Main Wing (Floor 1)", firmware_version: str = "2.4.0-esp32") -> bool:
        ...
        try:
            ...
            # Load existing metadata if available to increment sessions
            ...
            for anc_id in all_anchors:
                anchors_dict[anc_id] = {
                    "eta": round(learner.anchor_eta[anc_id], 4),
                    "bias": round(learner.anchor_bias[anc_id], 4),
                    "samples": learner.anchor_samples[anc_id]
                }
            ...
            payload = {
                ...
                "total_samples": learner.samples_learned_count,
                "last_updated": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
            }
