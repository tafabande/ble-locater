...
from sklearn.ensemble import RandomForestRegressor, ExtraTreesRegressor, GradientBoostingRegressor, HistGradientBoostingRegressor, AdaBoostRegressor, BaggingRegressor, StackingRegressor, VotingRegressor, RandomForestClassifier, ExtraTreesClassifier, HistGradientBoostingClassifier, AdaBoostClassifier, BaggingClassifier, VotingClassifier, GradientBoostingClassifier
from sklearn.svm import SVR
from sklearn.neighbors import KNeighborsRegressor, KNeighborsClassifier
from sklearn.neural_network import MLPRegressor, MLPClassifier
from sklearn.linear_model import RidgeCV, BayesianRidge, ElasticNet
from sklearn.tree import DecisionTreeRegressor
from sklearn.model_selection import train_test_split, cross_val_score, StratifiedKFold, KFold, GroupKFold, GroupShuffleSplit, StratifiedGroupKFold
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score, median_absolute_error, accuracy_score, classification_report, confusion_matrix, f1_score
...
BASE_FEATURE_COLUMNS = ['packet_count', 'scan_duration_ms', 'rssi_mean', 'rssi_median', 'rssi_min', 'rssi_max', 'rssi_std', 'rssi_variance', 'rssi_range', 'rssi_p05', 'rssi_p10', 'rssi_p25', 'rssi_p75', 'rssi_p90', 'rssi_p95', 'rssi_iqr', 'rssi_p90_10_range', 'rssi_mad', 'rssi_snr', 'rssi_skewness', 'rssi_kurtosis', 'rssi_delta_mean', 'rssi_delta_std', 'rssi_delta_max', 'observed_adv_interval', 'adv_interval_std', 'path_loss_free_space', 'path_loss_indoor', 'rssi_mean_to_std_ratio', 'rssi_median_mean_diff']
TEMPORAL_FEATURE_COLUMNS = ['rssi_slope', 'rssi_trend_strength', 'rssi_ema_diff', 'rssi_first_half_mean', 'rssi_second_half_mean', 'rssi_half_diff', 'rssi_autocorrelation', 'rssi_energy']
CROSS_WINDOW_FEATURE_COLUMNS = ['rssi_mean_delta', 'rssi_mean_slope_3w', 'rssi_mean_slope_5w', 'rssi_rolling_mean_3w', 'rssi_rolling_std_3w', 'rssi_rolling_mean_5w', 'rssi_rolling_std_5w', 'rssi_ema_cross_window', 'rssi_velocity', 'rssi_acceleration', 'signal_stability_index', 'rssi_rolling_mean_10w', 'rssi_rolling_std_10w', 'rssi_motion_direction', 'rssi_snr_rolling_5w']
ENVIRONMENTAL_INTERACTION_COLUMNS = ['obstacle_factor', 'rssi_x_height', 'pathloss_div_obstacle_factor', 'pathloss_free_div_obstacle_factor', 'rssi_div_height']
PHYSICAL_METADATA_COLUMNS = ['height_m']
ALL_FEATURE_COLUMNS = BASE_FEATURE_COLUMNS + TEMPORAL_FEATURE_COLUMNS + CROSS_WINDOW_FEATURE_COLUMNS + PHYSICAL_METADATA_COLUMNS + ENVIRONMENTAL_INTERACTION_COLUMNS
...
def instantiate_candidates() -> dict:
    rf = RandomForestRegressor(n_estimators=400, max_depth=20, min_samples_split=3, min_samples_leaf=2, max_features='sqrt', random_state=RANDOM_STATE, n_jobs=-1)
    et = ExtraTreesRegressor(n_estimators=400, max_depth=20, min_samples_split=3, min_samples_leaf=2, max_features='sqrt', random_state=RANDOM_STATE, n_jobs=-1)
    gb = GradientBoostingRegressor(n_estimators=300, learning_rate=0.05, max_depth=5, subsample=0.8, min_samples_leaf=5, random_state=RANDOM_STATE)
    hgb = HistGradientBoostingRegressor(max_iter=300, learning_rate=0.05, max_depth=6, min_samples_leaf=10, l2_regularization=0.1, random_state=RANDOM_STATE)
    ...
    candidates = {'Random Forest (400 Trees)': rf, 'Extra Trees (400 Trees)': et, 'Gradient Boosting (300)': gb, 'Hist Gradient Boosting': hgb, 'AdaBoost Regressor': ada, 'KNN Regressor (k=7)': knn, 'MLP Neural Network': mlp, 'SVR (RBF Kernel)': svr, 'Bayesian Ridge': bayesian, 'ElasticNet': elastic, 'Bagging Ensemble': bagging}
    if HAS_XGBOOST:
        xgb = XGBRegressor(n_estimators=400, learning_rate=0.05, max_depth=6, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0, min_child_weight=3, gamma=0.1, random_state=RANDOM_STATE, n_jobs=-1, verbosity=0)
        xgb_tuned = XGBRegressor(n_estimators=600, learning_rate=0.03, max_depth=8, subsample=0.7, colsample_bytree=0.7, reg_alpha=0.5, reg_lambda=2.0, min_child_weight=5, gamma=0.2, random_state=RANDOM_STATE, n_jobs=-1, verbosity=0)
    ...
    if HAS_LIGHTGBM:
        lgbm = LGBMRegressor(n_estimators=400, learning_rate=0.05, max_depth=6, num_leaves=31, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0, min_child_samples=10, random_state=RANDOM_STATE, n_jobs=-1, verbose=-1)
    ...
    if HAS_XGBOOST:
        voting_estimators.append(('xgb', XGBRegressor(n_estimators=200, max_depth=5, random_state=RANDOM_STATE, n_jobs=-1, verbosity=0)))
...
def evaluate_error_tolerances(y_true, y_pred) -> dict:
    try:
        ...
        return {'within_10cm': round(np.sum(errors <= 0.1) / total * 100, 2), 'within_25cm': round(np.sum(errors <= 0.25) / total * 100, 2), 'within_50cm': round(np.sum(errors <= 0.5) / total * 100, 2), 'within_75cm': round(np.sum(errors <= 0.75) / total * 100, 2), 'within_100cm': round(np.sum(errors <= 1.0) / total * 100, 2), 'within_150cm': round(np.sum(errors <= 1.5) / total * 100, 2)}
    except Exception:
        return {'within_10cm': 0.0, 'within_25cm': 0.0, 'within_50cm': 0.0, 'within_75cm': 0.0, 'within_100cm': 0.0, 'within_150cm': 0.0}
...
def evaluate_extended_metrics(y_true, y_pred) -> dict:
    try:
        ...
        for dist in unique_dists:
            mask = np.abs(y_true - dist) < 0.05
            if np.sum(mask) > 0:
                per_distance_mae[f'{dist:.1f}m'] = round(float(np.mean(np.abs(y_true[mask] - y_pred[mask]))), 4)
        return {'mape': round(mape, 2), 'max_error': round(max_error, 4), 'p95_error': round(p95_error, 4), 'explained_variance': round(explained_var, 4), 'per_distance_mae': per_distance_mae}
    except Exception as e:
        logger.warning(f'Extended metrics computation failed: {e}')
        return {'mape': 0.0, 'max_error': 0.0, 'p95_error': 0.0, 'explained_variance': 0.0, 'per_distance_mae': {}}
...
def evaluate_path_loss_baseline(X_test: np.ndarray, y_test: np.ndarray, feature_cols: list) -> dict:
    try:
        ...
        return {'name': 'Classical Path-Loss Physics Baseline', 'mae': round(baseline_mae, 4), 'rmse': round(baseline_rmse, 4), 'r2': round(baseline_r2, 4)}
    except Exception as e:
...
def train_model(df: pd.DataFrame, tune_hyperparams: bool=False, eval_mode: str='session', prune_features: bool=False, progress_callback=None) -> dict:
    ...
    if eval_mode in ('session', 'strict_session', 'balanced_session') and n_unique_sessions >= 2:
        ...
        try:
            ...
            if n_uniques > 1 and len(y_clean) >= 10:
                X_train, X_test, y_train, y_test = train_test_split(X_clean, y_clean, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y_strat)
            else:
    ...
    for idx, (name, raw_model) in enumerate(candidates.items(), 1):
        ...
        if progress_callback:
            progress_callback(f'Training {name}', progress_pct, {'windows': len(X_clean), 'features': len(feature_cols), 'sessions': n_unique_sessions})
        start_event = {'type': 'model_status', 'model_name': name, 'index': idx, 'total': total_candidates, 'status': 'TRAINING', 'percent': progress_pct}
        ...
        try:
            ...
            steps = []
            if prune_features:
                steps.append(('selector', SelectFromModel(RandomForestRegressor(n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1), threshold=0.001)))
            else:
            ...
            print(f"  -> Test MAE: {mae:.4f}m | RMSE: {rmse:.4f}m | R2: {r2:.4f} | <=25cm: {tols['within_25cm']}% | <=50cm: {tols['within_50cm']}%")
            trained_models[name] = pipeline
            tournament_results.append({'name': name, 'mae': mae, 'rmse': rmse, 'r2': r2, 'med_ae': med, 'tolerances': tols, 'ext_metrics': ext_metrics, 'cv': cv_results, 'y_pred': y_pred})
            success_event = {'type': 'model_status', 'model_name': name, 'index': idx, 'total': total_candidates, 'status': 'SUCCESS', 'mae': round(mae, 4), 'rmse': round(rmse, 4), 'r2': round(r2, 4), 'cv_mae': round(cv_results.get('cv_mae_mean', mae), 4), 'percent': progress_pct}
            print(json.dumps(success_event), flush=True)
        except Exception as e:
            logger.error(f'Candidate {name} failed: {e}')
            print(f'  [FAILED]: {e}')
            fail_event = {'type': 'model_status', 'model_name': name, 'index': idx, 'total': total_candidates, 'status': 'FAILED', 'error': str(e), 'percent': progress_pct}
            print(json.dumps(fail_event), flush=True)
    if not tournament_results:
        rf_fallback = Pipeline([('selector', 'passthrough'), ('scaler', 'passthrough'), ('model', RandomForestRegressor(n_estimators=100, random_state=RANDOM_STATE))])
        rf_fallback.fit(X_train, y_train)
        y_pred = rf_fallback.predict(X_test)
        mae = float(mean_absolute_error(y_test, y_pred))
        tols = evaluate_error_tolerances(y_test, y_pred)
        trained_models['Fallback RF'] = rf_fallback
        tournament_results.append({'name': 'Fallback RF', 'mae': mae, 'rmse': float(np.sqrt(mean_squared_error(y_test, y_pred))), 'r2': float(r2_score(y_test, y_pred)), 'med_ae': float(median_absolute_error(y_test, y_pred)), 'tolerances': tols, 'ext_metrics': evaluate_extended_metrics(y_test, y_pred), 'cv': {}, 'y_pred': y_pred})
    ...
    for i, res in enumerate(tournament_results):
        ...
        print(f"  {i + 1:<5} {res['name']:<30} {res['mae']:8.4f} {res['rmse']:8.4f} {res['r2']:7.4f} {cv_str:>10} {res['tolerances']['within_25cm']:6.1f}%{marker}")
    print(f"{'=' * 75}")
    print(f"\n{'=' * 75}")
    ...
    improvement_pct = max(0.0, (path_loss_mae - champion['mae']) / max(path_loss_mae, 1e-05) * 100.0)
    ...
    try:
        ...
        perm_res = permutation_importance(champion_model, X_test, y_test, n_repeats=10, random_state=RANDOM_STATE, n_jobs=-1)
    ...
    metrics = {'train_mae': round(float(mean_absolute_error(y_train, champion_model.predict(X_train))), 4), 'test_mae': round(champion['mae'], 4), 'test_rmse': round(champion['rmse'], 4), 'test_r2': round(champion['r2'], 4), 'test_median_ae': round(champion['med_ae'], 4), 'tolerances': champion['tolerances'], 'extended': ext_metrics, 'champion_name': champion_name, 'cv_metrics': champion.get('cv', {}), 'n_outliers_removed': n_outliers, 'n_features_active': int(np.sum(keep_mask)), 'n_features_total': len(feature_cols)}
    return {'model': champion_model, 'scaler': scaler_obj, 'metrics': metrics, 'importances': perm_importances, 'y_test': y_test, 'y_pred_test': champion['y_pred'], 'y_train': y_train, 'feature_cols': feature_cols, 'all_feature_cols': feature_cols, 'keep_mask': keep_mask.tolist(), 'model_type': champion_name, 'tournament': [{k: v for k, v in res.items() if k != 'y_pred'} for res in tournament_results]}

def instantiate_classification_candidates() -> dict:
    candidates = {'Random Forest Classifier': RandomForestClassifier(n_estimators=400, max_depth=20, min_samples_split=3, random_state=RANDOM_STATE, n_jobs=-1), 'Extra Trees Classifier': ExtraTreesClassifier(n_estimators=400, max_depth=20, min_samples_split=3, random_state=RANDOM_STATE, n_jobs=-1), 'Hist Gradient Boosting Clf': HistGradientBoostingClassifier(max_iter=300, learning_rate=0.05, max_depth=6, random_state=RANDOM_STATE), 'Gradient Boosting Clf': GradientBoostingClassifier(n_estimators=200, learning_rate=0.05, max_depth=5, subsample=0.8, random_state=RANDOM_STATE), 'KNN Classifier (k=7)': KNeighborsClassifier(n_neighbors=7, weights='distance', n_jobs=-1), 'MLP Classifier': MLPClassifier(hidden_layer_sizes=(128, 64, 32), activation='relu', solver='adam', max_iter=500, early_stopping=True, random_state=RANDOM_STATE), 'AdaBoost Classifier': AdaBoostClassifier(n_estimators=200, learning_rate=0.05, random_state=RANDOM_STATE), 'Bagging Classifier': BaggingClassifier(n_estimators=200, max_samples=0.8, max_features=0.8, random_state=RANDOM_STATE, n_jobs=-1)}
    if HAS_XGBOOST:
...
def train_zone_classifier(df: pd.DataFrame, eval_mode='random', progress_callback=None) -> dict:
    ...
    for name, model in candidates.items():
        ...
        try:
            ...
            clf_results.append({'name': name, 'accuracy': round(acc * 100, 2), 'f1_weighted': round(f1, 4), 'cv_accuracy_mean': cv_acc_mean, 'cv_accuracy_std': cv_acc_std})
            if f1 > best_f1:
                best_f1 = f1
                best_name = name
                best_model = model
    ...
    for i, res in enumerate(clf_results):
        ...
        print(f"  {i + 1:<5} {res['name']:<30} {res['accuracy']:6.1f}% {res['f1_weighted']:6.4f} {res['cv_accuracy_mean']:6.1f}%{marker}")
    print(f"\n{'=' * 75}")
    ...
    try:
        ...
        cm = np.array([])
    return {'model': best_model, 'scaler': scaler, 'zone_accuracy': round(float(accuracy_score(y_test, best_y_pred)) * 100, 2), 'f1_weighted': round(best_f1, 4), 'champion_name': best_name, 'feature_cols': feature_cols, 'zone_labels': ZONE_LABELS, 'zone_to_int': zone_to_int, 'int_to_zone': int_to_zone, 'y_test_labels': y_test_labels, 'y_pred_labels': y_pred_labels, 'confusion_matrix': cm, 'classification_report': report, 'tournament': clf_results}
...
def generate_plots(result: dict, output_dir: str, zone_result: dict=None):
    try:
        ...
        vals = [tols.get('within_10cm', 0), tols.get('within_25cm', 0), tols.get('within_50cm', 0), tols.get('within_75cm', 0), tols.get('within_100cm', 0), tols.get('within_150cm', 0)]
        ...
        for thresh, label in [(0.25, '25cm'), (0.5, '50cm'), (1.0, '1m')]:
    ...
    try:
        ...
        if os.path.exists(json_log_path):
            try:
                ...
                history = []
        if not history:
            history = [{'timestamp': '2026-07-31T20:00:00', 'phase': 'Phase 1 (Legacy Initial Baseline)', 'raw_files': 12, 'total_windows': 5420, 'features_count': 30, 'champion_model': 'RandomForestRegressor', 'test_mae': 0.6724, 'test_rmse': 0.8912, 'test_r2': 0.371, 'zone_accuracy': None, 'physics_baseline_mae': 2.5, 'improvement_over_physics_pct': 73.1, 'methodology': 'Initial raw packet aggregation, basic RSSI mean/std features, random train/test split.'}, {'timestamp': '2026-08-01T16:00:00', 'phase': 'Phase 2 (Feature Expansion & Temporal Windowing)', 'raw_files': 22, 'total_windows': 14200, 'features_count': 38, 'champion_model': 'XGBoost (Deep Tuned)', 'test_mae': 0.2643, 'test_rmse': 0.5118, 'test_r2': 0.8688, 'zone_accuracy': 88.5, 'physics_baseline_mae': 2.5, 'improvement_over_physics_pct': 89.4, 'methodology': 'Added cross-window velocity, acceleration, rolling averages, and RSSI IQR.'}, {'timestamp': '2026-08-03T12:00:00', 'phase': 'Phase 3 (Session GroupKFold & 60 BLE Domain Features)', 'raw_files': 34, 'total_windows': 24555, 'features_count': 60, 'champion_model': 'CatBoost Regressor', 'test_mae': 0.2315, 'test_rmse': 0.4102, 'test_r2': 0.9102, 'zone_accuracy': 94.2, 'physics_baseline_mae': 2.45, 'improvement_over_physics_pct': 90.6, 'methodology': '60 BLE domain features (packet_loss_rate, 6 RSSI histogram power density bins), IsolationForest signal space anomaly filtering.'}]
        metrics = metadata.get('metrics', {})
        zone_meta = metadata.get('zone_classification', {})
        current_entry = {'timestamp': metadata.get('trained_at', datetime.datetime.now().isoformat()), 'phase': 'Phase 4 (Zero-Leakage Pipeline CV & Physics Baseline Benchmark)', 'raw_files': 34, 'total_windows': metadata.get('train_samples', 0) + metadata.get('test_samples', 0), 'features_count': len(metadata.get('all_feature_cols', [])), 'champion_model': metadata.get('champion_model', 'Unknown'), 'test_mae': metrics.get('test_mae', 0.0), 'test_rmse': metrics.get('test_rmse', 0.0), 'test_r2': metrics.get('test_r2', 0.0), 'zone_accuracy': zone_meta.get('zone_accuracy', None), 'physics_baseline_mae': 2.45, 'improvement_over_physics_pct': round(max(0.0, (2.45 - metrics.get('test_mae', 0.0)) / 2.45 * 100.0), 1), 'methodology': 'In-Fold Pipeline feature selection & scaling, session GroupKFold CV, zero-leakage composite score selection.'}
...
def save_model(result: dict, output_dir: str, zone_result: dict=None):
    try:
        ...
        if zone_result and zone_result.get('model'):
            ...
            metadata['zone_classification'] = {'champion_classifier': zone_result['champion_name'], 'zone_accuracy': zone_result['zone_accuracy'], 'f1_weighted': zone_result.get('f1_weighted', 0), 'zone_labels': zone_result['zone_labels'], 'zone_to_int': zone_result['zone_to_int'], 'feature_cols': zone_result['feature_cols'], 'tournament': zone_result['tournament']}
...
def main():
    ...
    parser.add_argument('--mode', type=str, choices=['regression', 'classification', 'both'], default='both', help='Training mode: regression only, classification (zones) only, or both (default)')
    ...
    if result:
        ...
        meta = {'zone_classification': {'champion_classifier': zone_result['champion_name'], 'zone_accuracy': zone_result['zone_accuracy'], 'zone_labels': zone_result['zone_labels'], 'tournament': zone_result['tournament'], 'trained_at': datetime.datetime.now().isoformat()}}
