"""
Sim2Real Dissertation Experiment & Cascaded Pipeline Benchmark Suite
======================================================================
Implements:
  1. Real Dataset Label Harmonization (0.68m->0.7m, 3.36m->3.4m, 4.6m->4.5m, 5.27m->5.3m)
  2. The 3-Model Sim2Real Research Experiment:
     - Model A: Real Data Only (Baseline)
     - Model B: Synthetic Data Only (Ablation - Sim2Real Gap)
     - Model C: Sim2Real Augmented & Fine-Tuned (Proposed)
  3. The 3-Stage Cascaded Pipeline:
     [RSSI] -> [ML Regression] -> [Kalman Correction] -> [Zone Classifier]
"""
...
def harmonize_distance_labels(df: pd.DataFrame) -> pd.DataFrame:
    """Harmonizes raw 3D hypotenuse distance labels into canonical ground truth presets."""
    ...
    def canonicalize(val):
        ...
        v = float(val)
        if abs(v - 0.68) < 0.1: return 0.7
        if abs(v - 1.06) < 0.1: return 1.1
        if abs(v - 1.88) < 0.1: return 1.9
        if abs(v - 3.36) < 0.1: return 3.4
        if abs(v - 4.45) < 0.2 or abs(v - 4.6) < 0.2: return 4.5
        if abs(v - 5.27) < 0.2 or abs(v - 5.29) < 0.2: return 5.3
        return round(v, 1)
...
def generate_synthetic_benchmark_data(n_samples=5000):
    """Generates ray-traced synthetic observations matching physical path-loss equations if synthetic CSV is missing."""
    np.random.seed(42)
    ...
    syn_df = pd.DataFrame({
        ...
        "distance_m_clean": distances,
        "rssi_mean": rssi_mean,
        "rssi_median": rssi_mean + np.random.normal(0, 0.5, size=n_samples),
        "rssi_min": rssi_mean - 3.0,
        "rssi_max": rssi_mean + 3.0,
        "rssi_std": np.abs(np.random.normal(2.0, 0.5, size=n_samples)),
        "path_loss_indoor": distances + np.random.normal(0, 0.2, size=n_samples),
        "session_id": "SYNTHETIC_SIM"
    })
    return syn_df
...
def main():
    ...
    # 1. Load and Harmonize Real Data
    ...
    # Select feature columns common to both datasets
    ...
    # Split Real Data into Train (80%) and Test (20%) using Session Holdout
    ...
    # ──────────────────────────────────────────────────────────────────
    # 3. RUN THE 3 EXPERIMENTAL MODELS
    # ──────────────────────────────────────────────────────────────────
    print("\n----------------------------------------------------------------------")
    ...
    # Model C: Synthetic Pre-training + Real Data Augmentation (Proposed)
    ...
    # 4. Cascaded Kalman Correction Simulation
    print("\n[CASCADED PIPELINE] Applying Kalman Correction Layer to Raw Predictions...")
    # Apply exponential smoothing / Kalman correction to Model C predictions
    ...
    # ──────────────────────────────────────────────────────────────────
    # 6. GENERATE DISSERTATION STUDY REPORT
    # ──────────────────────────────────────────────────────────────────
