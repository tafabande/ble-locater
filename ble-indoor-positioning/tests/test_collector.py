...
def test_init_csv_file():
    """Verifies that init_csv_file creates the directory and writes headers."""
...
def test_process_line_observation():
    """Verifies that observation JSON lines (including advanced statistics) are parsed and written to the CSV."""
    with tempfile.TemporaryDirectory() as tmpdir:
        obs_path = os.path.join(tmpdir, "obs.csv")
        raw_path = os.path.join(tmpdir, "raw.csv")
        
        # Extended headers list
        obs_headers = [
        ...
        # Test input payload with advanced C++ stats
        obs_payload = {
            "type": "observation",
            "anchor_id": "A1",
            "timestamp": 123456,
            ...
            "percentile_25": -57.5,
            "percentile_75": -54.5,
            "packet_loss_estimate": 0.1,
            "max_consecutive_gap_ms": 120
        }
        ...
        with open(obs_path, "r", newline="", encoding="utf-8") as f:
            ...
            assert row[0] == "A1"
            assert row[1] == "123456"
            assert float(row[2]) > 0 # host timestamp
            assert row[3] == "52:06:26:03:01:DA"
            assert row[4] == "10"
            assert row[5] == "1000"
            assert row[6] == "-55.5"
            assert row[14] == "-56.0" # rssi_median
            assert row[15] == "-56" # rssi_mode
            assert row[16] == "-0.1542" # skewness
            assert row[17] == "2.145" # kurtosis
            assert row[18] == "-57.5" # percentile_25
            assert row[19] == "-54.5" # percentile_75
            assert row[20] == "0.1" # packet_loss_estimate
            assert row[21] == "120" # max_consecutive_gap_ms


def test_process_line_observation_fallback_keys():
    """Verifies that the collector successfully falls back to short keys (anchor, device)."""
    with tempfile.TemporaryDirectory() as tmpdir:
        obs_path = os.path.join(tmpdir, "obs.csv")
        raw_path = os.path.join(tmpdir, "raw.csv")
        ...
        # Test input payload using fallback keys
        obs_payload = {
            "type": "observation",
            "anchor": "A2",
            "timestamp": 654321,
            "device": "52:06:26:03:01:DA",
            "packet_count": 8
        }
        ...
        with open(obs_path, "r", newline="", encoding="utf-8") as f:
            ...
            row = rows[1]
            assert row[0] == "A2" # resolved from 'anchor'
            assert row[1] == "654321"
            assert row[3] == "52:06:26:03:01:DA" # resolved from 'device'
            assert row[4] == "8"


def test_process_line_raw():
    """Verifies that raw packet JSON lines are parsed and written to the CSV."""
    with tempfile.TemporaryDirectory() as tmpdir:
        obs_path = os.path.join(tmpdir, "obs.csv")
        raw_path = os.path.join(tmpdir, "raw.csv")
...
def test_process_line_invalid():
    """Verifies that malformed or non-JSON lines are handled without throwing exceptions."""
    with tempfile.TemporaryDirectory() as tmpdir:
        obs_path = os.path.join(tmpdir, "obs.csv")
        raw_path = os.path.join(tmpdir, "raw.csv")
