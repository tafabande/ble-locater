...
def decode_value(data: bytes) -> dict:
    ...
    # 4. Standard Numeric Interpretations (based on length)
    length = len(data)
    ...
    # Two-byte values (e.g. 16-bit short)
    if length >= 2:
    ...
    # Four-byte values (e.g. 32-bit int, float)
    if length >= 4:
    ...
    # Eight-byte values (e.g. 64-bit float/double)
    if length >= 8:
        try:
...
async def main():
    ...
    try:
        ...
        # 2. Perform Authentication Write
        log(f"[AUTH] Attempting authentication to write-char {AUTH_CHAR_UUID}...")
        auth_outcome = "Unknown"
        try:
            # Write password as UTF-8 bytes with response
            await asyncio.wait_for(
                client.write_gatt_char(AUTH_CHAR_UUID, AUTH_PASSWORD.encode("utf-8"), response=True),
                timeout=5.0
            )
        ...
        # 3. Read Custom Telemetry Characteristic
        log(f"[READ] Reading telemetry characteristic {TELEMETRY_CHAR_UUID}...")
        try:
            val_bytes = await asyncio.wait_for(
                client.read_gatt_char(TELEMETRY_CHAR_UUID),
                timeout=5.0
            )
            log(f"[READ] Success: Retrieved {len(val_bytes)} bytes.")
            
            # Decode using multi-representation decoder
