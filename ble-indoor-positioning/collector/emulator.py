...
def generate_rssi(mean: float, noise: float) -> int:
    """Generates a simulated RSSI value with normal distribution."""
...
def calculate_stats(packets: list[dict], anchor_id: str, mac: str, window_start_time_ms: int) -> dict:
    """Computes all the observation window statistics on a list of raw packets."""
    count = len(packets)
    if count == 0:
        return {}
    ...
    # Consecutive packets stats
    rssi_delta_mean = 0.0
    advertising_interval_ms = 0.0
    max_consecutive_gap_ms = 1000  # Default to window duration if count <= 1
    lost_packets = 0
    ...
    if count > 1:
        ...
        for i in range(1, count):
            ...
            gaps.append(gap)
            # Estimate lost packets based on standard 100ms interval
    ...
    return {
        "type": "observation",
        "anchor_id": anchor_id,
        "timestamp": window_start_time_ms,
        "device_mac": mac,
        "packet_count": count,
        ...
        "rssi_delta_mean": round(rssi_delta_mean, 2),
        "advertising_interval_ms": round(advertising_interval_ms, 2),
        
        "rssi_median": round(rssi_median, 2),
        "rssi_mode": rssi_mode,
        "skewness": round(skewness, 4),
        "kurtosis": round(kurtosis, 4),
        "percentile_25": round(percentile_25, 2),
        "percentile_75": round(percentile_75, 2),
        "packet_loss_estimate": round(packet_loss_estimate, 4),
        "max_consecutive_gap_ms": max_consecutive_gap_ms
    }

def main() -> None:
    parser = argparse.ArgumentParser(description="ESP32 BLE Anchor Node - Hardware Emulator")
    parser.add_argument("--mode", "-m", type=str, choices=["NORMAL", "RAW", "DUAL"], default="NORMAL",
                        help="Operation mode of the emulated ESP32 (default: NORMAL)")
    ...
    parser.add_argument("--interval", "-i", type=float, default=100.0,
                        help="Simulated advertising interval of the tag in milliseconds (default: 100.0)")
    parser.add_argument("--duration", "-d", type=float, default=None,
                        help="Run for N seconds, then exit. If not set, runs indefinitely.")
            ...
            if now - last_window_time >= 1.0:
                ...
                if args.mode in ["NORMAL", "DUAL"] and packet_buffer:
                    ...
                    if obs:
                        ...
                        # Output human-readable debug logs to match ESP32 output behavior
