...
def run_audit():
    ...
    for d in all_distances:
        ...
        split_audit_data.append({'Distance (m)': d, 'Train Windows': tr_c, 'Train %': round(tr_pct, 2), 'Test Windows': te_c, 'Test %': round(te_pct, 2), 'Distribution Delta (%)': round(diff_pct, 2)})
    ...
    for sess, s_df in test_df_copy.groupby('session_id'):
        ...
        session_results.append({'Session File': sess, 'Windows': len(s_df), 'Distance (m)': round(dist_mean, 2), 'Obstacle Type': obs_str, 'MAE (m)': round(s_mae, 4), 'RMSE (m)': round(s_rmse, 4), 'Session R2': round(s_r2, 4) if not np.isnan(s_r2) else 'Single Target'})
    ...
    try:
        axes[1, 0].boxplot(dist_groups, tick_labels=[f'{d}m' for d in all_distances], patch_artist=True, boxprops=dict(facecolor='#2ecc71', color='#27ae60'))
    except TypeError:
        axes[1, 0].boxplot(dist_groups, patch_artist=True, boxprops=dict(facecolor='#2ecc71', color='#27ae60'))
    ...
    if 'obstacle_type' in df.columns:
        ...
        try:
            axes[1, 1].boxplot(obs_groups, tick_labels=[str(o) for o in obs_types], patch_artist=True, boxprops=dict(facecolor='#f39c12', color='#d35400'))
        except TypeError:
            axes[1, 1].boxplot(obs_groups, patch_artist=True, boxprops=dict(facecolor='#f39c12', color='#d35400'))
