...
class ConfigManager:
    DEFAULT_CONFIG = {'baud_rate': DEFAULT_BAUD_RATE, 'auto_scan_interval_ms': 1500, 'stream_endpoint_url': 'http://localhost:8000/api/observation', 'stream_enabled': True, 'buffer_flush_size': 200, 'window_size': 50, 'stride': 10, 'target_mac_filter': '52:06:26:03:01:DA', 'target_windows_goal': 2500}
    ...
    def _worker_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                packet_json = {'timestamp': int(row[0]), 'anchor': row[1], 'mac': row[2], 'rssi': int(row[3]), 'name': row[4]}
                try:
...
class DatasetWriter:
    ...
    def start_session(self, distance, obstacle, obstacle_type, height_m=1.0, motion='stationary', dirty_mode='', target_mac=''):
        ...
        self.unflushed_count = 0
        self.metadata_info = {'session_filename': filename, 'start_timestamp': self.session_start_time, 'end_timestamp': None, 'distance_m': distance, 'height_m': height_m, 'motion': motion, 'obstacle': obstacle, 'obstacle_type': obstacle_type, 'dirty_environment_mode': dirty_mode, 'target_mac': target_mac, 'total_samples': 0}
        self.csv_file = open(self.dataset_path, 'w', newline='', encoding='utf-8')
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow(['timestamp', 'anchor', 'mac', 'rssi', 'name', 'distance_m', 'obstacle', 'obstacle_type', 'height_m', 'motion', 'duplicate_candidate'])
        return filename
...
class DatasetAuditor:
    ...
    def run_audit(self):
        ...
        if file_durations:
            ...
            sample_rate_hz = 20.0
        return {'distance': {'records': dist_samples, 'target': self.target_samples}, 'height': {'records': height_samples, 'target': 10000}, 'motion': {'records': motion_samples, 'target': 10000}, 'obstacle': {'records': obs_samples, 'target': 15000}, 'anchor': {'records': anchor_samples, 'target': self.target_samples}, 'sample_rate_hz': round(sample_rate_hz, 1), 'total_samples': total_samples}
...
class BLECollector:
    ...
    def __init__(self, root):
        ...
        self.writer_mgr = DatasetWriter(DATA_DIR, buffer_flush_size=self.config_mgr.get('buffer_flush_size', 50))
        self.auditor_mgr = DatasetAuditor(DATA_DIR, target_samples=self.config_mgr.get('target_windows_goal', 2500) * 10)
        self.streamer_mgr = NetworkStreamer(endpoint_url=self.config_mgr.get('stream_endpoint_url', 'http://localhost:8000/api/observation'), enabled=self.config_mgr.get('stream_enabled', True))
    ...
    def setup_styles(self):
        ...
        self.style.configure('Card.TLabelframe', background=card_bg, bordercolor=card_border, darkcolor=card_border, lightcolor=card_border, relief='solid', borderwidth=1, padding=12)
        self.style.configure('Card.TLabelframe.Label', background=card_bg, foreground=accent_blue, font=(font_primary, 11, 'bold'))
        ...
        self.style.configure('Primary.TButton', font=(font_primary, 10, 'bold'), background=accent_green, foreground='#11111b', padding=(14, 8))
        self.style.map('Primary.TButton', background=[('disabled', '#45475a'), ('active', '#94e2d5')], foreground=[('disabled', '#7f849c')])
        self.style.configure('Warning.TButton', font=(font_primary, 10, 'bold'), background=accent_yellow, foreground='#11111b', padding=(14, 8))
        self.style.map('Warning.TButton', background=[('disabled', '#45475a'), ('active', '#f9e2af')], foreground=[('disabled', '#7f849c')])
        self.style.configure('Danger.TButton', font=(font_primary, 10, 'bold'), background=accent_red, foreground='#11111b', padding=(14, 8))
        self.style.map('Danger.TButton', background=[('disabled', '#45475a'), ('active', '#f38ba8')], foreground=[('disabled', '#7f849c')])
        self.style.configure('Secondary.TButton', font=(font_primary, 9), background='#313244', foreground=fg_text, padding=(8, 4))
        self.style.map('Secondary.TButton', background=[('active', '#45475a')])
        self.style.configure('Pill.TButton', font=(font_primary, 9), background='#313244', foreground='#cdd6f4', padding=(6, 3))
        self.style.map('Pill.TButton', background=[('active', accent_blue)], foreground=[('active', '#11111b')])
        self.style.configure('TEntry', fieldbackground='#313244', foreground=fg_text, insertcolor=fg_text)
        self.style.configure('Treeview', background='#1e1e2e', foreground='#cdd6f4', fieldbackground='#1e1e2e', rowheight=26, font=(font_primary, 9))
        self.style.configure('Treeview.Heading', background='#313244', foreground='#89b4fa', font=(font_primary, 10, 'bold'))
    ...
    def build_gui(self):
        ...
        subtitle = ttk.Label(header_frame, text='ESP32 RSSI & Metadata Acquisition System (Modular V3)', style='Subtext.TLabel', background='#181825')
        ...
        self.flash_btn = ttk.Button(flash_frame, text='⚡ Load / Flash ESP32 Code', style='Primary.TButton', command=self.start_flash_firmware)
        self.flash_btn.pack(side='left', padx=(0, 10))
        self.autoflash_chk = ttk.Checkbutton(flash_frame, text='Auto-Flash on Connect', variable=self.auto_flash_var, command=self.on_autoflash_toggle)
        ...
        for d_val in ['0.1', '0.5', '1.0', '2.0', '3.2', '5.0']:
            btn = ttk.Button(dist_input_frame, text=f'{d_val}m', style='Pill.TButton', command=lambda val=d_val: self.set_preset_distance(val))
            btn.pack(side='left', padx=2)
        rand_btn = ttk.Button(dist_input_frame, text='🎲 Random Dist', style='Secondary.TButton', command=self.set_random_distance)
        ...
        height_presets = [('Floor (0.0m)', '0.0'), ('Waist (1.0m)', '1.0'), ('Desk (1.4m)', '1.4'), ('Head (1.7m)', '1.7')]
        for label, h_val in height_presets:
            btn = ttk.Button(height_frame, text=label, style='Pill.TButton', command=lambda val=h_val: self.set_preset_height(val))
            btn.pack(side='left', padx=2)
        ...
        self.motion_combo = ttk.Combobox(motion_frame, values=['stationary', 'approaching', 'moving_away'], state='readonly', width=18)
        self.motion_combo.set('stationary')
        self.motion_combo.pack(side='left', padx=(0, 10))
        ttk.Label(motion_frame, text='🚶 Stationary = still | Approaching = walking toward beacon | Moving Away = walking from beacon', style='Subtext.TLabel').pack(side='left')
        ttk.Label(exp_box, text='Environment / Dirty Data Mode:').grid(row=3, column=0, sticky='w', padx=5, pady=5)
        self.dirty_mode_combo = ttk.Combobox(exp_box, values=['Clean / Direct Line-of-Sight (LOS)', 'WiFi Interference (2.4GHz Heavy)', 'Concrete Wall', 'WiFi + Concrete Wall', 'Human Body Absorption', 'Dynamic Movement / Multipath', 'Phone Orientation Switch', 'Custom / Manual'], state='readonly', width=32)
        ...
        self.obstacle_type_combo = ttk.Combobox(obs_type_frame, values=['None', 'WiFi Interference', 'Concrete Wall', 'WiFi + Concrete Wall', 'Human Body', 'Wooden Door', 'Glass Window', 'Metal Shield', 'Multipath Movement', 'Orientation Change', 'Custom...'], width=22)
        ...
        ttk.Label(exp_box, text="💡 Tip: Collecting arbitrary distances, heights, and 'Dirty Data' ensures maximum ML model robustness.", style='Subtext.TLabel').grid(row=6, column=0, columnspan=4, sticky='w', padx=5, pady=(5, 0))
        ...
        self.console = tk.Text(console_inner, height=10, bg='#11111b', fg='#cdd6f4', insertbackground='#cdd6f4', selectbackground='#45475a', font=(getattr(self, 'font_mono', 'Consolas'), 9), relief='flat', state='disabled', yscrollcommand=scrollbar.set)
        ...
        self.audit_summary_lbl = ttk.Label(audit_top, text='Scanning raw dataset files across 5 experimental dimensions...', style='Subtext.TLabel', foreground='#f9e2af')
        ...
        self.audit_dim_combo = ttk.Combobox(audit_ctrls, values=['Distance (m)', 'Height / Elevation (m)', 'Motion Mode', 'Environment / Obstacle', 'Anchor Nodes'], state='readonly', width=22)
        ...
        ttk.Button(audit_ctrls, text='🔄 Refresh Deficit Audit', style='Secondary.TButton', command=self.refresh_dataset_audit).pack(side='left')
        tree_frame = ttk.Frame(audit_box)
        tree_frame.pack(fill='x', expand=True)
        tree_scroll = ttk.Scrollbar(tree_frame, orient='vertical')
        tree_scroll.pack(side='right', fill='y')
        self.audit_tree = ttk.Treeview(tree_frame, columns=('category', 'current_samp', 'target_samp', 'missing_samp', 'est_min', 'status'), show='headings', height=5, yscrollcommand=tree_scroll.set)
    ...
    def flash_worker(self, port):
        ...
        cmd = [sys.executable, '-m', 'esptool', '--chip', 'esp32', '--port', port, '--baud', '115200', '--before', 'default-reset', '--after', 'hard-reset', 'write-flash', '--flash-mode', 'dio', '--flash-freq', '40m', '--flash-size', 'detect', '0x1000', BOOTLOADER_BIN, '0x8000', PARTITION_BIN, '0x10000', APP_BIN]
        try:
    ...
    def render_audit_dim_data(self, audit_data):
        ...
        dim_map = {'Distance (m)': 'distance', 'Height / Elevation (m)': 'height', 'Motion Mode': 'motion', 'Environment / Obstacle': 'obstacle', 'Anchor Nodes': 'anchor'}
        ...
        for category, current in records.items():
            ...
            if hasattr(self, 'audit_tree'):
                self.audit_tree.insert('', 'end', values=(cat_label, f'{current:,}', f'{target_goal:,}', f'{missing:,}', f'~{est_mins} mins', status))
        if hasattr(self, 'audit_summary_lbl'):
    ...
    def on_dirty_preset_change(self, event=None):
        ...
        mapping = {'Clean / Direct Line-of-Sight (LOS)': ('No', 'None'), 'WiFi Interference (2.4GHz Heavy)': ('Yes', 'WiFi Interference'), 'Concrete Wall': ('Yes', 'Concrete Wall'), 'WiFi + Concrete Wall': ('Yes', 'WiFi + Concrete Wall'), 'Human Body Absorption': ('Yes', 'Human Body'), 'Dynamic Movement / Multipath': ('Yes', 'Multipath Movement'), 'Phone Orientation Switch': ('Yes', 'Orientation Change')}
        if mode in mapping:
    ...
    def start_collection(self):
        ...
        try:
            filename = self.writer_mgr.start_session(distance=distance, obstacle=obstacle, obstacle_type=obstacle_type, height_m=height_m, motion=motion, dirty_mode=dirty_mode, target_mac=target_mac)
            ...
            self.reader_thread = threading.Thread(target=self.serial_reader, args=(distance, obstacle, obstacle_type, height_m, motion, target_mac), daemon=True)
    ...
    def serial_reader(self, distance, obstacle, obstacle_type, height_m=1.0, motion='stationary', target_mac=''):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                last_rssi = rssi_val
                last_ts = ts_val
                row = [timestamp, anchor, mac, rssi, name, distance, obstacle, obstacle_type, height_m, motion, duplicate_candidate]
    ...
    def stop_collection(self):
        ...
        messagebox.showinfo('Collection Complete', f'Successfully recorded {total_samples} BLE samples!\n\nSaved to:\n{dataset_path}\n\nMetadata written to dataset_info.json')
    ...
    def lock_inputs(self, lock=True):
        ...
        if lock:
            self.obstacle_type_combo.config(state='disabled')
        elif self.obstacle_combo.get() == 'Yes':
            self.obstacle_type_combo.config(state='normal')
        else:
            self.obstacle_type_combo.config(state='disabled')
